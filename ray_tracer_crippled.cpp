...
void Scene::CalculateRightAndUp()
{
	...
	double theta = 0.5 * fov * 3.14159265 / 180;
	double u = tan(theta);
	double r = ((double)screen_x / screen_y) * u;
						...
						)
{
	...
	double a = pow(ray.v[0], 2) + pow(ray.v[1], 2) + pow(ray.v[2], 2);
	double b = 2 * (ecx * ray.v[0]) + 2 * (ecy * ray.v[1]) + 2 * (ecz * ray.v[2]);
	double c = pow(ecx, 2) + pow(ecy, 2) + pow(ecz, 2) - pow(s.radius, 2);
	if (pow(b, 2) - 4 * (a * c) < 0) {
		return false;
	}
...
void Scene::CastRay(const Vector3 &ray,float &r,float &g,float &b)
{
	...
	if(closest_hit_sphere != -1)
	{
		...
		for (int l = 0; l < num_point_lights; ++l) {
			Vector3 vector_to_the_light = pl[l] - closest_point;
			vector_to_the_light.Normalize();

			double angle = DotProduct(vector_to_the_light, closest_normal);
			if (angle > 0) {
				double diffuse_ratio = angle;
				r += diffuse_ratio * s[closest_hit_sphere].diffuse[0];
				g += diffuse_ratio * s[closest_hit_sphere].diffuse[1];
				b += diffuse_ratio * s[closest_hit_sphere].diffuse[2];
			}

			if (angle > 0) {
				Vector3 vector_to_eye = eye - closest_point;
				vector_to_eye.Normalize();
				Vector3 SumVector = vector_to_eye + vector_to_the_light;
				SumVector.Normalize();
				double pos = DotProduct(SumVector, closest_normal);
				if (pos > 0) {
					double specularExponet = s[closest_hit_sphere].specular_exponent;
					double specRatio = pow(pos, specularExponet);
					r += specRatio * s[closest_hit_sphere].specular[0];
					g += specRatio * s[closest_hit_sphere].specular[1];
					b += specRatio * s[closest_hit_sphere].specular[2];
